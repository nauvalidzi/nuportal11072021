 ...
 * Page class
 */
class KelurahansAdd extends Kelurahans
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "KelurahansAdd";
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$loaded) { // Record not loaded
                    ...
                    $this->terminate("KelurahansList"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "KelurahansList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "KelurahansView") {
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if ($curVal != "") {
                ...
                $this->kecamatan_id->ViewValue = $this->kecamatan_id->Lookup !== null && is_array($this->kecamatan_id->Lookup->Options) ? $curVal : null;
            }
            if ($this->kecamatan_id->ViewValue !== null) { // Load from cache
                $this->kecamatan_id->EditValue = array_values($this->kecamatan_id->Lookup->Options);
                if ($this->kecamatan_id->ViewValue == "") {
                ...
                if ($curVal == "") {
                    ...
                    $filterWrk = "`id`" . SearchString("=", $this->kecamatan_id->CurrentValue, DATATYPE_STRING, "");
                }
                $sqlWrk = $this->kecamatan_id->Lookup->getSql(true, $filterWrk, '', $this, false, true);
                $rswrk = Conn()->executeQuery($sqlWrk)->fetchAll(\PDO::FETCH_BOTH);
                $ari = count($rswrk);
                if ($ari > 0) { // Lookup values found
                    $arwrk = $this->kecamatan_id->Lookup->renderViewRow($rswrk[0]);
    ...
    protected function addRow($rsold = null)
    {
        ...
        // kecamatan_id
        $this->kecamatan_id->setDbValueDef($rsnew, $this->kecamatan_id->CurrentValue, "", false);
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("KelurahansList"), "", $this->TableVar, true);
