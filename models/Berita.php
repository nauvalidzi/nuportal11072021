...
class Berita extends DbTable
{
    ...
    public function __construct()
    {
        ...
        // slug
        $this->slug = new DbField('berita', 'berita', 'x_slug', 'slug', '`slug`', '`slug`', 200, 255, -1, false, '`slug`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        $this->slug->Sortable = true; // Allow sort
        $this->slug->CustomMsg = $Language->FieldPhrase($this->TableVar, $this->slug->Param, "CustomMsg");
        $this->Fields['slug'] = &$this->slug;

        // deskripsi
        $this->deskripsi = new DbField('berita', 'berita', 'x_deskripsi', 'deskripsi', '`deskripsi`', '`deskripsi`', 201, 65535, -1, false, '`deskripsi`', false, false, false, 'FORMATTED TEXT', 'TEXTAREA');
        ...
        // foto
        $this->foto = new DbField('berita', 'berita', 'x_foto', 'foto', '`foto`', '`foto`', 200, 255, -1, true, '`foto`', false, false, false, 'IMAGE', 'FILE');
        ...
        // status
        $this->status = new DbField('berita', 'berita', 'x_status', 'status', '`status`', '`status`', 202, 7, -1, false, '`status`', false, false, false, 'FORMATTED TEXT', 'RADIO');
        $this->status->Sortable = true; // Allow sort
        switch ($CurrentLanguage) {
            case "en":
                $this->status->Lookup = new Lookup('status', 'berita', false, '', ["","","",""], [], [], [], [], [], [], '', '');
                break;
            default:
                $this->status->Lookup = new Lookup('status', 'berita', false, '', ["","","",""], [], [], [], [], [], [], '', '');
                break;
        ...
        // created_at
        $this->created_at = new DbField('berita', 'berita', 'x_created_at', 'created_at', '`created_at`', CastDateFieldForLike("`created_at`", 0, "DB"), 135, 19, 0, false, '`created_at`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // updated_at
        $this->updated_at = new DbField('berita', 'berita', 'x_updated_at', 'updated_at', '`updated_at`', CastDateFieldForLike("`updated_at`", 0, "DB"), 135, 19, 0, false, '`updated_at`', false, false, false, 'FORMATTED TEXT', 'TEXT');
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("BeritaList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        } elseif ($pageName == "BeritaEdit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "BeritaAdd") {
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("BeritaView", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("BeritaView", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "BeritaAdd?" . $this->getUrlParm($parm);
        } else {
            $url = "BeritaAdd";
        }
        return $this->addMasterUrl($url);
    ...
    {
        $url = $this->keyUrl("BeritaEdit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("BeritaAdd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("BeritaDelete", $this->getUrlParm());
    }
...
SORTHTML;
    ...
    public function renderListRow()
    {
        ...
        if ($this->foto->UseColorbox) {
            ...
            $this->foto->LinkAttrs["data-rel"] = "berita_x_foto";
