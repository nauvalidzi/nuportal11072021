...
loadjs.ready("head", function () {
    ...
    fkodeposlist = currentForm = new ew.Form("fkodeposlist", "list");
    fkodeposlist.formKeyCountName = '<?= $Page->FormKeyCountName ?>';
    loadjs.done("fkodeposlist");
});
</script>
<style>
...
<div class="card ew-card ew-grid<?php if ($Page->isAddOrEdit()) { ?> ew-grid-add-edit<?php } ?> kodepos">
<form name="fkodeposlist" id="fkodeposlist" class="form-inline ew-form ew-list-form" action="<?= CurrentPageUrl(false) ?>" method="post">
<?php if (Config("CHECK_TOKEN")) { ?>
<input type="hidden" name="<?= $TokenNameKey ?>" value="<?= $TokenName ?>"><!-- CSRF token name -->
<input type="hidden" name="<?= $TokenValueKey ?>" value="<?= $TokenValue ?>"><!-- CSRF token value -->
<?php } ?>
<input type="hidden" name="t" value="kodepos">
<div id="gmp_kodepos" class="<?= ResponsiveTableClass() ?>card-body ew-grid-middle-panel">
<?php if ($Page->TotalRecords > 0 || $Page->isGridEdit()) { ?>
<table id="tbl_kodeposlist" class="table ew-table"><!-- .ew-table -->
<thead>
...
while ($Page->RecordCount < $Page->StopRecord) {
    ...
    if ($Page->RecordCount >= $Page->StartRecord) {
        ...
        $Page->RowAttrs->merge(["data-rowindex" => $Page->RowCount, "id" => "r" . $Page->RowCount . "_kodepos", "data-rowtype" => $Page->RowType]);
