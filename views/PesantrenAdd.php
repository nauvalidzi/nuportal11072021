...
loadjs.ready("head", function () {
    ...
    fpesantrenadd = currentForm = new ew.Form("fpesantrenadd", "add");
    ...
    // Validate form
    fpesantrenadd.validate = function () {
    ...
    // Form_CustomValidate
    fpesantrenadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fpesantrenadd.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fpesantrenadd.lists.propinsi = <?= $Page->propinsi->toClientList($Page) ?>;
...
$Page->showMessage();
?>
<form name="fpesantrenadd" id="fpesantrenadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<input type="<?= $Page->kode->getInputTextType() ?>" data-table="pesantren" data-field="x_kode" name="x_kode" id="x_kode" size="30" maxlength="255" value="<?= $Page->kode->EditValue ?>"<?= $Page->kode->editAttributes() ?> aria-describedby="x_kode_help">
...
<script>
loadjs.ready(["fpesantrenadd", "datetimepicker"], function() {
    ew.createDateTimePicker("fpesantrenadd", "x_nspptglmulai", {"ignoreReadonly":true,"useCurrent":false,"format":7});
});
</script>
<?php } ?>
</span>
...
<script>
loadjs.ready(["fpesantrenadd", "datetimepicker"], function() {
    ew.createDateTimePicker("fpesantrenadd", "x_nspptglakhir", {"ignoreReadonly":true,"useCurrent":false,"format":7});
});
</script>
<?php } ?>
</span>
...
<input type="hidden" name="fa_x_dokumennspp" id= "fa_x_dokumennspp" value="0">
...
<script>
loadjs.ready(["fpesantrenadd", "datetimepicker"], function() {
    ew.createDateTimePicker("fpesantrenadd", "x_tglakta", {"ignoreReadonly":true,"useCurrent":false,"format":7});
});
</script>
<?php } ?>
</span>
...
<?php if ($Page->_userid->Visible) { // userid ?>
    <div id="r__userid" class="form-group row">
        <label id="elh_pesantren__userid" for="x__userid" class="<?= $Page->LeftColumnClass ?>"><?= $Page->_userid->caption() ?><?= $Page->_userid->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<input type="hidden" data-table="pesantren" data-field="x__userid" data-hidden="1" name="x__userid" id="x__userid" value="<?= HtmlEncode($Page->_userid->CurrentValue) ?>">
<?php } else { ?>
<span id="el_pesantren__userid">
    <select
        id="x__userid"
        ...
        data-select2-id="pesantren_x__userid"
        data-table="pesantren"
...
<script>
loadjs.ready("head", function() {
    var el = document.querySelector("select[data-select2-id='pesantren_x__userid']"),
        options = { name: "x__userid", selectId: "pesantren_x__userid", language: ew.LANGUAGE_ID, dir: ew.IS_RTL ? "rtl" : "ltr" };
    options.dropdownParent = $(el).closest("#ew-modal-dialog, #ew-add-opt-dialog")[0];
    Object.assign(options, ew.vars.tables.pesantren.fields._userid.selectOptions);
    ew.createSelect(options);
});
</script>
</span>
...
<?php
    if (in_array("fasilitasusaha", explode(",", $Page->getCurrentDetailTable())) && $fasilitasusaha->DetailAdd) {
?>
...
<?php } ?>
<?php
    if (in_array("pendidikanumum", explode(",", $Page->getCurrentDetailTable())) && $pendidikanumum->DetailAdd) {
?>
<?php if ($Page->getCurrentDetailTable() != "") { ?>
...
<?php } ?>
<?php
    if (in_array("pengasuhpppria", explode(",", $Page->getCurrentDetailTable())) && $pengasuhpppria->DetailAdd) {
?>
<?php if ($Page->getCurrentDetailTable() != "") { ?>
...
<?php } ?>
<?php
    if (in_array("pengasuhppwanita", explode(",", $Page->getCurrentDetailTable())) && $pengasuhppwanita->DetailAdd) {
?>
<?php if ($Page->getCurrentDetailTable() != "") { ?>
...
<?php } ?>
<?php
    if (in_array("kitabkuning", explode(",", $Page->getCurrentDetailTable())) && $kitabkuning->DetailAdd) {
?>
<?php if ($Page->getCurrentDetailTable() != "") { ?>
...
<?php } ?>
<?php
    if (in_array("fasilitaspesantren", explode(",", $Page->getCurrentDetailTable())) && $fasilitaspesantren->DetailAdd) {
?>
<?php if ($Page->getCurrentDetailTable() != "") { ?>
