...
loadjs.ready("head", function () {
    ...
    fkelurahansedit = currentForm = new ew.Form("fkelurahansedit", "edit");
    ...
    // Validate form
    fkelurahansedit.validate = function () {
    ...
    // Form_CustomValidate
    fkelurahansedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fkelurahansedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fkelurahansedit.lists.kecamatan_id = <?= $Page->kecamatan_id->toClientList($Page) ?>;
    loadjs.done("fkelurahansedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fkelurahansedit" id="fkelurahansedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<input type="hidden" data-table="kelurahans" data-field="x_id" data-hidden="1" name="o_id" id="o_id" value="<?= HtmlEncode($Page->id->OldValue ?? $Page->id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
