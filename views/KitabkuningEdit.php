...
// Page object
$KitabkuningEdit = &$Page;
?>
<script>
var currentForm, currentPageID;
var fkitabkuningedit;
loadjs.ready("head", function () {
    var $ = jQuery;
    // Form object
    currentPageID = ew.PAGE_ID = "edit";
    fkitabkuningedit = currentForm = new ew.Form("fkitabkuningedit", "edit");
    ...
    // Validate form
    fkitabkuningedit.validate = function () {
    ...
    // Form_CustomValidate
    fkitabkuningedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fkitabkuningedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fkitabkuningedit.lists.pid = <?= $Page->pid->toClientList($Page) ?>;
    fkitabkuningedit.lists.pelaksanaan = <?= $Page->pelaksanaan->toClientList($Page) ?>;
    fkitabkuningedit.lists.metode = <?= $Page->metode->toClientList($Page) ?>;
    loadjs.done("fkitabkuningedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fkitabkuningedit" id="fkitabkuningedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->id->Visible) { // id ?>
    <div id="r_id" class="form-group row">
        <label id="elh_kitabkuning_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id->caption() ?><?= $Page->id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->id->cellAttributes() ?>>
<span id="el_kitabkuning_id">
<span<?= $Page->id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id->getDisplayValue($Page->id->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="kitabkuning" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<script>
loadjs.ready(["fkitabkuningedit"], function() {
    fkitabkuningedit.createAutoSuggest(Object.assign({"id":"x_pid","forceSelect":false}, ew.vars.tables.kitabkuning.fields.pid.autoSuggestOptions));
});
</script>
