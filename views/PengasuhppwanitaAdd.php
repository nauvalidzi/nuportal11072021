...
// Page object
$PengasuhppwanitaAdd = &$Page;
?>
<script>
var currentForm, currentPageID;
var fpengasuhppwanitaadd;
loadjs.ready("head", function () {
    var $ = jQuery;
    // Form object
    currentPageID = ew.PAGE_ID = "add";
    fpengasuhppwanitaadd = currentForm = new ew.Form("fpengasuhppwanitaadd", "add");
    ...
    fpengasuhppwanitaadd.addFields([
    ...
    // Validate form
    fpengasuhppwanitaadd.validate = function () {
    ...
    // Form_CustomValidate
    fpengasuhppwanitaadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fpengasuhppwanitaadd.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fpengasuhppwanitaadd.lists.pid = <?= $Page->pid->toClientList($Page) ?>;
    loadjs.done("fpengasuhppwanitaadd");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fpengasuhppwanitaadd" id="fpengasuhppwanitaadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<script>
loadjs.ready(["fpengasuhppwanitaadd"], function() {
    fpengasuhppwanitaadd.createAutoSuggest(Object.assign({"id":"x_pid","forceSelect":false}, ew.vars.tables.pengasuhppwanita.fields.pid.autoSuggestOptions));
});
</script>
...
<script>
loadjs.ready(["fpengasuhppwanitaadd", "datetimepicker"], function() {
    ew.createDateTimePicker("fpengasuhppwanitaadd", "x_tglawalorganisasi", {"ignoreReadonly":true,"useCurrent":false,"format":7});
});
</script>
<?php } ?>
</span>
...
<script>
loadjs.ready(["fpengasuhppwanitaadd", "datetimepicker"], function() {
    ew.createDateTimePicker("fpengasuhppwanitaadd", "x_tglmenjabat", {"ignoreReadonly":true,"useCurrent":false,"format":0});
});
</script>
<?php } ?>
</span>
...
<?php if ($Page->dokumen->Visible) { // dokumen ?>
    <div id="r_dokumen" class="form-group row">
        <label id="elh_pengasuhppwanita_dokumen" class="<?= $Page->LeftColumnClass ?>"><?= $Page->dokumen->caption() ?><?= $Page->dokumen->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->dokumen->cellAttributes() ?>>
<span id="el_pengasuhppwanita_dokumen">
<div id="fd_x_dokumen">
<div class="input-group">
    <div class="custom-file">
        <input type="file" class="custom-file-input" title="<?= $Page->dokumen->title() ?>" data-table="pengasuhppwanita" data-field="x_dokumen" name="x_dokumen" id="x_dokumen" lang="<?= CurrentLanguageID() ?>"<?= $Page->dokumen->editAttributes() ?><?= ($Page->dokumen->ReadOnly || $Page->dokumen->Disabled) ? " disabled" : "" ?> aria-describedby="x_dokumen_help">
