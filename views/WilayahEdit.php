...
loadjs.ready("head", function () {
    ...
    fwilayahedit = currentForm = new ew.Form("fwilayahedit", "edit");
    ...
    // Validate form
    fwilayahedit.validate = function () {
    ...
    // Form_CustomValidate
    fwilayahedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fwilayahedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fwilayahedit.lists.iduser = <?= $Page->iduser->toClientList($Page) ?>;
    fwilayahedit.lists.idprovinsis = <?= $Page->idprovinsis->toClientList($Page) ?>;
    fwilayahedit.lists.idkabupatens = <?= $Page->idkabupatens->toClientList($Page) ?>;
    loadjs.done("fwilayahedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fwilayahedit" id="fwilayahedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php } ?>
    ...
    <input type="hidden" data-table="wilayah" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
<?php if (!$Page->IsModal) { ?>
