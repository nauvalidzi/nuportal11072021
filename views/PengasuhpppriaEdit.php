...
// Page object
$PengasuhpppriaEdit = &$Page;
?>
<script>
var currentForm, currentPageID;
var fpengasuhpppriaedit;
loadjs.ready("head", function () {
    var $ = jQuery;
    // Form object
    currentPageID = ew.PAGE_ID = "edit";
    fpengasuhpppriaedit = currentForm = new ew.Form("fpengasuhpppriaedit", "edit");
    ...
    // Validate form
    fpengasuhpppriaedit.validate = function () {
    ...
    // Form_CustomValidate
    fpengasuhpppriaedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fpengasuhpppriaedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fpengasuhpppriaedit.lists.pid = <?= $Page->pid->toClientList($Page) ?>;
    loadjs.done("fpengasuhpppriaedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fpengasuhpppriaedit" id="fpengasuhpppriaedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->id->Visible) { // id ?>
    <div id="r_id" class="form-group row">
        <label id="elh_pengasuhpppria_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id->caption() ?><?= $Page->id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->id->cellAttributes() ?>>
<span id="el_pengasuhpppria_id">
<span<?= $Page->id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id->getDisplayValue($Page->id->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="pengasuhpppria" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<script>
loadjs.ready(["fpengasuhpppriaedit"], function() {
    fpengasuhpppriaedit.createAutoSuggest(Object.assign({"id":"x_pid","forceSelect":false}, ew.vars.tables.pengasuhpppria.fields.pid.autoSuggestOptions));
});
</script>
...
<script>
loadjs.ready(["fpengasuhpppriaedit", "datetimepicker"], function() {
    ew.createDateTimePicker("fpengasuhpppriaedit", "x_tglawalorganisasi", {"ignoreReadonly":true,"useCurrent":false,"format":7});
});
</script>
<?php } ?>
</span>
...
<script>
loadjs.ready(["fpengasuhpppriaedit", "datetimepicker"], function() {
    ew.createDateTimePicker("fpengasuhpppriaedit", "x_tglmenjabat", {"ignoreReadonly":true,"useCurrent":false,"format":7});
});
</script>
<?php } ?>
</span>
